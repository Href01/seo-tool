...
export default function SerpPage() {
  ...
  function posClass(p: number | null) {
    if ((p ?? 99) <= 3) return 'bg-[var(--up-bg)] text-[var(--up)]'
    if ((p ?? 99) <= 10) return 'bg-amber-100 text-amber-700'
    return 'bg-[var(--subtle)] text-[var(--text-2)]'
  }

  return (
    <Page>
      <PageHeader title="Analyse SERP" subtitle="Qui domine le top des résultats · détection des plateformes · concurrents réels" />
      ...
      <Card className="mb-6">
        ...
        {cached !== null && !error && data && (
          <div className="mt-4 border-t border-[var(--line)] pt-3">
            <CacheMeta cached={cached} fetchedAt={fetchedAt} timeAgo={timeAgo} extra={`${data.length} résultats`} />
          </div>
        )}
      </Card>
      ...
      {data && insights && (
        <div className="mb-6 grid gap-3 sm:grid-cols-3">
          <StatCard label="Domaines uniques" value={insights.uniqueDomains} sub={`Sur ${data.length} résultats`} />
          <StatCard label="Vrais concurrents" value={insights.realCompetitors} sub="Sites e-commerce / business" accent />
          <StatCard label="Plateformes" value={insights.platforms} sub="Insta, FB, YT… (opportunité)" />
        </div>
      )}
        ...
        <div className="space-y-2.5">
          {data.map((r, i) => {
            ...
            return (
              <div
                key={i}
                className={`rounded-2xl border bg-[var(--card)] p-4 transition-colors ${
                  isPlatform ? 'border-[var(--line)] opacity-70' : 'border-[var(--line)] hover:border-[var(--text-3)]'
                }`}
              >
                <div className="flex items-start gap-3">
                  <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-sm font-bold tnum ${posClass(r.position)}`}>
                    {r.position ?? '—'}
                  </div>
                  <div className="min-w-0 flex-1">
                    <a href={r.url} target="_blank" rel="noreferrer" className="block truncate font-medium text-[var(--text)] hover:text-[var(--crimson)]">
                      {r.title || r.url}
                    </a>
                    <div className="mt-0.5 flex items-center gap-2">
                      <span className="text-xs text-[var(--text-2)]">{r.domain}</span>
                      {isPlatform && <Pill tone="neutral">Plateforme</Pill>}
                    </div>
      ...
      {!data && !loading && !error && (
        <EmptyState icon="📊" title="Espionne le SERP" hint="Découvre qui ranke et repère les opportunités face aux plateformes." />
      )}
    </Page>
  )
}
